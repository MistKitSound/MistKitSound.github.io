...
const DEFAULT_TRACKS_FALLBACK = [
  ...
  "owo.ogg",
  "Rage.ogg",
  "Ship Of Those Who Was Murdered By God Will.ogg",
  "speedwalk.ogg"
];
    ...
    return data
      .map((track) => {
        if (typeof track === "string") {
          ...
          return { name, file: `./content/audio/${track}`, category: null };
        }
...
function buildTrackList(list, tracks, showAllTracks, moreBtn) {
  ...
  tracks.forEach((track, index) => {
    ...
    if (track.category === "piano") {
      item.classList.add("piano-piece");
    }
...
function getSectionFromHash() {
  ...
  const allowed = new Set(["profile", "license", "faq", "tracks"]);
  if (allowed.has(raw)) {
    return raw;
  }
  return "profile";
...
function showSection(sectionName, updateHash) {
  ...
  if (updateHash && sectionName !== "profile") {
